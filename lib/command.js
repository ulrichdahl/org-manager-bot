...
module.exports = class Command {
    ...
    static async handleReaction(event, reaction, user) {
        ...
        if (reaction.partial) {
            console.log('Partial reaction received, fetching it!');
        ...
        if (reaction.message.partial) {
            console.log('Partial message received, fetching it!');
            // If the message this reaction belongs to was removed the fetching might result in an API error, which we need to handle
            try {
        ...
        if (data === false) {
            console.log('No data found for reaction, trying to fetch it...');
            await reaction.fetch();
        }
        var data = Command.decodeFooter(reaction.message);
        if (data === false) {
            console.log('No data found for reaction so removing it', reaction);
            ...
            if (command && command.executeReaction) {
                ...
                console.log('Command does not have an executeReaction method', data.command);
            }
        }
        else {
            console.log('Data does contain a command name', data);
        }
    }
}
